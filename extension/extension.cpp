...
bool BSPPeek::SDK_OnLoad(char *error, size_t maxlen, bool late)
{
	...
	{
		smutils->LogMessage(myself, "Engine displacement reader disabled: %s", dispEngErr);
	}
	else
	{
	...
	smutils->LogMessage(myself,
						...
						"  leafs %d/%d",
						base, BSPData::DebugGetOff("numbrushes"), BSPData::DebugGetOff("map_brushes"), BSPData::DebugGetOff("numplanes"),
						BSPData::DebugGetOff("map_planes"), BSPData::DebugGetOff("numleafs"), BSPData::DebugGetOff("map_leafs"));
	return true;
}
...
bool BSPPeek::QueryRunning(char *error, size_t maxlen)
{
	return true;
}

void BSPPeek::OnCoreMapStart(edict_t *pEdictList, int edictCount, int clientMax)
{
	BSPData::OnMapStart();
	BSPProps::OnMapClear();
