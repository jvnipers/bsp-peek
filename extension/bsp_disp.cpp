...
bool ReadLump(FILE *f, const uint8_t *header, int lump,
...
bool GetBaseQuadCorners(const uint8_t *facesLump, size_t facesLen,
                        const int32_t *surfedges, size_t numSurfedges,
                        const uint16_t *edges, size_t numEdges,
                        const Vec3 *vertexes, size_t numVertexes, int faceIdx,
                        const Vec3 &startPos, Vec3 outCorners[4]) {
...
inline Vec3 BaseAt(const Vec3 corners[4], float u, float v) {
  ...
  return {
      mu * mv * corners[0].x + u * mv * corners[1].x + u * v * corners[2].x +
          mu * v * corners[3].x,
      mu * mv * corners[0].y + u * mv * corners[1].y + u * v * corners[2].y +
          mu * v * corners[3].y,
      mu * mv * corners[0].z + u * mv * corners[1].z + u * v * corners[2].z +
          mu * v * corners[3].z,
  };
}

void BuildDispMesh(const Vec3 corners[4], int power,
                   const uint8_t *dispVertLump, size_t dispVertLen,
                   int dispVertStart, Displacement &out) {
  int side = (1 << power) + 1;
  int numVerts = side * side;
  if (dispVertStart < 0 ||
      (size_t)(dispVertStart + numVerts) * kSizeofDispVert > dispVertLen)
    return;
...
bool InitEngine(IGameConfig *gc, char *err, size_t errLen) {
  ...
  if (!gc->GetAddress("g_DispCollTreeCount", &addr) || !addr) {
    std::snprintf(err, errLen,
                  "gamedata Address 'g_DispCollTreeCount' did not resolve");
    return false;
  }
  ...
  if (engine::SZ_DISPCOLL_TREE <= 0 || engine::SZ_DISPCOLL_TRI <= 0) {
    std::snprintf(err, errLen,
                  "gamedata Keys for CDispCollTree layout incomplete");
    return false;
  }
  return true;
}
...
bool EnsureLoaded(const char *mapname, const char *bspPath, char *errOut,
  ...
  if (!mapname || !bspPath)
    return false;
  if (disk::g_loadedMap[0] &&
      std::strncmp(disk::g_loadedMap, mapname, sizeof(disk::g_loadedMap)) == 0)
    return true;
...
bool LoadFromMap(const char *bspPath, char *err, size_t errLen) {
  ...
  std::vector<uint8_t> vertsLump, edgesLump, surfedgesLump, facesLump,
      dispInfoLump, dispVertsLump;
  if (!disk::ReadLump(f, header.data(), disk::LUMP_VERTEXES, vertsLump) ||
      !disk::ReadLump(f, header.data(), disk::LUMP_EDGES, edgesLump) ||
      !disk::ReadLump(f, header.data(), disk::LUMP_SURFEDGES, surfedgesLump) ||
      !disk::ReadLump(f, header.data(), disk::LUMP_FACES, facesLump) ||
      !disk::ReadLump(f, header.data(), disk::LUMP_DISPINFO, dispInfoLump) ||
  ...
  const int32_t *surfedges =
      reinterpret_cast<const int32_t *>(surfedgesLump.data());
  ...
  for (size_t i = 0; i < numDisps; i++) {
    ...
    Vec3 corners[4];
    if (!disk::GetBaseQuadCorners(facesLump.data(), facesLump.size(), surfedges,
                                  numSurfedges, edges, numEdges, vertexes,
                                  numVerts, (int)mapFace, startPos, corners))
      continue;

    disk::Displacement d;
    disk::BuildDispMesh(corners, (int)power, dispVertsLump.data(),
                        dispVertsLump.size(), dispVertStart, d);
    if (!d.tris.empty()) {
...
int EngineCount() { return engine::g_pTreeCount ? *engine::g_pTreeCount : 0; }
...
int EngineDebugTreeInfo(int idx, char *buf, size_t bufLen) {
  if (!EngineReady() || idx < 0 || idx >= EngineCount()) {
    return std::snprintf(
        buf, bufLen, "engine disp tree idx %d unavailable (ready=%d count=%d)",
        idx, EngineReady() ? 1 : 0, EngineCount());
  }
  ...
  auto dumpHex = [](const void *p, char *out, size_t outLen, int n) {
    ...
    for (int i = 0; i < n && w + 4 < (int)outLen; i++)
      w += std::snprintf(out + w, outLen - w, "%s%02X",
                         (i && (i % 16 == 0)) ? "|" : (i ? " " : ""), b[i]);
  };
  ...
  return std::snprintf(buf, bufLen,
                       ...
                       "    tri0[%s]\n    vp0[%s]",
                       idx, (const void *)tree, power, vCnt, tCnt, mn.x, mn.y,
                       mn.z, mx.x, mx.y, mx.z, vp, tp, triHex, vertHex);
}
...
int EngineDiagnoseQuery(float x, float y, char *buf, size_t bufLen) {
  ...
  if (firstTree < 0)
    return std::snprintf(buf, bufLen,
                         "diag (%.3f,%.3f): 0/%d trees AABB-cover point", x, y,
                         treeCount);
  ...
  int trisBboxPass = 0, trisInvalidIdx = 0, trisDegen = 0, trisBaryFail = 0,
      trisHit = 0;
  ...
  for (int t = 0; t < trisCnt; t++) {
    ...
    float wa =
        ((v1.y - v2.y) * (x - v2.x) + (v2.x - v1.x) * (y - v2.y)) / denom;
    float wb =
        ((v2.y - v0.y) * (x - v2.x) + (v0.x - v2.x) * (y - v2.y)) / denom;
    float wc = 1.0f - wa - wb;
  ...
  return std::snprintf(
      ...
      x, y, treesAABB, firstTree, vertsCnt, trisCnt, trisBboxPass,
      trisInvalidIdx, trisDegen, trisBaryFail, trisHit, bestZ, bestWMin);
}
...
int DiskDebugDispInfo(int idx, char *buf, size_t bufLen) {
  if (idx < 0 || (size_t)idx >= disk::g_disps.size())
    return std::snprintf(buf, bufLen, "disp idx %d out of range (count=%d)",
                         idx, (int)disk::g_disps.size());
  const auto &d = disk::g_disps[idx];
  return std::snprintf(buf, bufLen,
                       ...
                       idx, d.sourceMapFace, d.sourcePower, (int)d.tris.size(),
                       d.bboxMins[0], d.bboxMins[1], d.bboxMins[2],
                       d.bboxMaxs[0], d.bboxMaxs[1], d.bboxMaxs[2],
                       d.corner0[0], d.corner0[1], d.corner0[2], d.corner1[0],
                       d.corner1[1], d.corner1[2], d.corner2[0], d.corner2[1],
                       d.corner2[2], d.corner3[0], d.corner3[1], d.corner3[2]);
}
