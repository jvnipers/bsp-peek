...
namespace BSPData
{
	...
	// the intersection of its sides' half-spaces (a point p is inside when normal.p <= dist for every side).
	// These accessors solve that polytope directly, so they don't share
	// FindBrushPairAtSeam's derived-AABB blind spot for box-optimized / bevel-padded brushes.
	...
	// every leaf-brush that actually contains pos.
	...
	// Fills [zMin, zMax]. Returns false if the column misses the brush footprint or the brush is degenerate.
	...
	// World-space polygon of brush side sideIdx,
	// built by clipping the side's plane against all other sides of the brush (the canonical brush-face winding).
	...
	// so the result matches what a real player-hull trace resolves against this one brush.
	...
	// Diagnostic:
	// dump each side of brushIdx in engine iteration order (the order CM_ClipBoxToBrush processes them, which the pixelsurf gate depends on):
	...
	// (collapsing in the cbrush-idx cache), so the regular pair finder misses them.
	...
	// Texturebug overhang.
	// Scans the cboxbrush_t table for a player-collidable (SOLID or PLAYERCLIP) box brush whose XY AABB contains samplePos,
	...
	// Among multiple qualifying boxes in the column, returns the one whose bottom edge is nearest BELOW samplePos.z
	...
	// FindBoxBrushOverhang + the face-selection velocity window in one call (the "texturebug" condition).
	...
	// Resolves the lower/upper brushes at the seam, then walks to the leaf just below seamZ (samplePos.xy, seamZ - 0.5)
	// and finds each brush's position in that leaf's brush list.
	// CSGO inverts leaf-visit order vs TF2, so the lower
	...
	//   outLowerPos/outUpperPos: index of lower/upper brush within that leaf's brush list (-1 if not present)
	// Returns true if both brushes were found AND both located in the leaf AND lowerPos < upperPos 
	// (engine visits lower first -> stalemate goes to the top).
	...
	// Brush AABB cache
	// Plugin should call once in OnMapStart so the first user-triggered query doesn't pay the build cost.
