...
// HullSweep: sweeps an AABB against the runtime props in a volume via IEngineTrace + IStaticPropMgr, works on prop-combine maps.
...
namespace BSPProps
{
	...
	// Unified world hull trace: sweep an AABB [mins,maxs] from start to end against the WHOLE collision world
	// via IEngineTrace::TraceRay - world brushes, displacements, and static props always participate.
	...
	// For RE the runtime field layout. (friction=+0 is the known anchor).
	...
	// Static-prop index hit by a ray from start to end, or -1 if the ray hits a brush / nothing.
	// Identifies the prop under an aim.
