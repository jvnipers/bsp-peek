...
namespace BSPLumps
{
	...
	namespace
	{
		...
		void ParseStaticProps(FILE *f)
		{
			...
			if (count <= 0 || (size_t)4 + (size_t)count * kSizeofGameLumpEntry > dir.size())
			{
				return;
			}
			...
			p += 4;
			if (dictCount < 0 || p + (size_t)dictCount * kStaticPropNameLen + 8 > b.size())
			{
				return;
			}
			...
			for (int i = 0; i < propCount; ++i)
			{
				...
				sp.fadeMinDist = (stride >= kSP_FadeMinDist + 4) ? ReadF32(q, kSP_FadeMinDist) : 0.0f;
				sp.fadeMaxDist = (stride >= kSP_FadeMaxDist + 4) ? ReadF32(q, kSP_FadeMaxDist) : 0.0f;
				...
				sp.forcedFadeScale = (stride >= kSP_ForcedFadeScale + 4) ? ReadF32(q, kSP_ForcedFadeScale) : 0.0f;
	...
	bool LoadFromMap(const char *mapname, const char *bspPath, char *err, size_t errLen)
	{
		...
		// Entities
		if (g.lumps[LUMP_ENTITIES].filelen > 0 && ReadFile(f, g.lumps[LUMP_ENTITIES].fileofs, g.lumps[LUMP_ENTITIES].filelen, raw))
		{
		...
		// Visibility
		ReadFile(f, g.lumps[LUMP_VISIBILITY].fileofs, g.lumps[LUMP_VISIBILITY].filelen, g.visLump);
		// Leaf water data:
		// derive the on-disk stride from the length (12 padded, 10 tight) so the count is right regardless of how the lump was packed.
		ReadFile(f, g.lumps[LUMP_LEAFWATERDATA].fileofs, g.lumps[LUMP_LEAFWATERDATA].filelen, g.leafWaterLump);
		...
		// Cubemaps
		ReadFile(f, g.lumps[LUMP_CUBEMAPS].fileofs, g.lumps[LUMP_CUBEMAPS].filelen, g.cubemapsLump);
		// Vertexes
		ReadFile(f, g.lumps[LUMP_VERTEXES].fileofs, g.lumps[LUMP_VERTEXES].filelen, g.vertexesLump);
		// Edges + Surfedges
		ReadFile(f, g.lumps[LUMP_EDGES].fileofs, g.lumps[LUMP_EDGES].filelen, g.edgesLump);
		ReadFile(f, g.lumps[LUMP_SURFEDGES].fileofs, g.lumps[LUMP_SURFEDGES].filelen, g.surfedgesLump);
		// Faces
		ReadFile(f, g.lumps[LUMP_FACES].fileofs, g.lumps[LUMP_FACES].filelen, g.facesLump);

		// Texinfo
		ReadFile(f, g.lumps[LUMP_TEXINFO].fileofs, g.lumps[LUMP_TEXINFO].filelen, g.texinfoLump);
		// Texdata
		ReadFile(f, g.lumps[LUMP_TEXDATA].fileofs, g.lumps[LUMP_TEXDATA].filelen, g.texdataLump);
		// Texdata string table + data
		ReadFile(f, g.lumps[LUMP_TEXDATA_STRING_TABLE].fileofs, g.lumps[LUMP_TEXDATA_STRING_TABLE].filelen, g.texdataStringTable);
		ReadFile(f, g.lumps[LUMP_TEXDATA_STRING_DATA].fileofs, g.lumps[LUMP_TEXDATA_STRING_DATA].filelen, g.texdataStringData);
		...
		{
			ReadFile(f, g.lumps[LUMP_WORLDLIGHTS_HDR].fileofs, g.lumps[LUMP_WORLDLIGHTS_HDR].filelen, g.worldlightsLump);
		}
	...
	int BSPVersion()
	{
		return g.loaded ? g.version : 0;
	}

	int BSPRevision()
	{
		return g.loaded ? g.revision : 0;
	}
	...
	bool HasLighting()
	{
		...
		return g.lumps[LUMP_LIGHTING].filelen > 0 || g.lumps[LUMP_LIGHTING_HDR].filelen > 0;
	}

	// Entities
	int EntityRawLen()
	{
		return g.loaded ? (int)g.entityRaw.size() : 0;
	}
	...
	int EntityCount()
	{
		return g.loaded ? (int)g.entities.size() : 0;
	}
	...
	int TexInfoFlags(int idx)
	{
		...
		return ReadI32(g.texinfoLump.data() + (size_t)idx * kSizeofTexInfo, kTexInfo_Flags);
	}
	...
	int TexInfoTexData(int idx)
	{
		...
		return ReadI32(g.texinfoLump.data() + (size_t)idx * kSizeofTexInfo, kTexInfo_TexData);
	}
	...
	int TexDataMaterialName(int texdataIdx, char *buf, int maxlen)
	{
		...
		int nameStringTableID = ReadI32(g.texdataLump.data() + (size_t)texdataIdx * kSizeofTexData, kTexData_NameStringTableID);
		if (nameStringTableID < 0 || (size_t)(nameStringTableID + 1) * 4 > g.texdataStringTable.size())
		{
			return 0;
		}
		int32_t stringOffset = ReadI32(g.texdataStringTable.data(), nameStringTableID * 4);
	...
	bool ClusterVisible(int cluster, int other)
	{
		...
		if (cluster < 0 || other < 0 || cluster >= numclusters || other >= numclusters)
		{
			return false;
		}
	...
	bool LeafWaterData(int idx, float &surfaceZ, float &minZ, int &surfaceTexInfo)
	{
		...
		const uint8_t *p = g.leafWaterLump.data() + (size_t)idx * (size_t)g.leafWaterStride;
	...
	int CubemapSize(int idx)
	{
		...
		return ReadI32(g.cubemapsLump.data() + (size_t)idx * kSizeofCubemap, kCubemap_Size);
	}
	...
	int StaticPropCount()
	{
		return g.loaded ? (int)g.staticProps.size() : 0;
	}

	int StaticPropVersion()
	{
		return g.loaded ? g.staticPropVersion : 0;
	}
